...
> 2026-09-12 更新：国家护盾档位改 2/3/4/5/6（15% 阶梯），好友攻击强化档位改 2/3/4/5/6（10% 阶梯，最多5级）；选将/商店羁绊显示中好友与国家 1 人=0 级。
...
| 机制 | 类型 | 触发方式 | 效果 | 状态 |
| --- | --- | --- | --- | --- |
| 国家连锁 | 阵营羁绊 | 同阵营英雄数达标（2/3/4/5/6） | 全阵营英雄获得护盾（15%/30%/45%/60%/75%）；君主（王）在场护盾额外+10% | ✅ 已实现 |
| 好友连锁·普通 | 武将关系 | 场上好友数量达标（2~6） | 连线特效 + 攻击强化（10%~50%） | ✅ 已实现 |
...
### 2. 君主强化
- 主公技（王/王，技能 id `200001`，职业 `shuai`）：所在阵营存在携带主公技的英雄时，**该阵营护盾额外 +10%**（`KingShieldBonusRate = 0.1`，每名王叠加一次）。
- 护盾走 `BuffManager.AddShield`（BuffId `300001` 护盾），不吃减伤、独立结算。

### 3. 配置位置
- 档位 / 倍率 / 时长：`CombatConst.FactionShieldCounts / FactionShieldRates / FactionShieldTime / MasterShieldSkillId / KingShieldBonusRate`
...
- 等级换算：档位从 2 人起（2人=1级，最多5级）；1 人时无加成即 0 级。商店/选将界面（`MySelectControl` 羁绊模式）中好友/国家显示"等级+名字"，1 人显示为 0 级。
...
- 兵种技能即职业默认技能：`JobConfig.SkillId`（如 王→200001、马→200005、弓→200007、士→200004、扇→200002、鼓→200016、刀→200003），由 `ConfigManager.PostModify` 注入每个英雄。
- 等级字段：`Skill.Level`（默认取配置 Lv=5），由 `Skill.SetLevel()` 修正；**当前仅记录等级、不缩放技能数值**，数值联动后续接入。
...
### 已实现（无需开发）
- [x] 国家连锁：同阵营护盾（`FactionShieldManager`）+ 君主（王）护盾加倍
...
## 七、验证点
1. 2/3/4/5/6 同阵营英雄分别获得 15%/30%/45%/60%/75% 护盾；王在场额外 +10%。
2. 2~6 名好友连线获得 10%~50% 攻击加成，连线特效正常，好友阵亡实时重算；1 人时 0 级无加成。
