...
| 机制 | 类型 | 触发方式 | 效果 | 状态 |
| --- | --- | --- | --- | --- |
| 国家连锁 | 阵营羁绊 | 同阵营英雄数达标（3/5/7/9） | 全阵营英雄获得护盾；君主（王）在场护盾加倍 | ✅ 已实现 |
...
### 2. 君主强化
- 主公技（王/王，技能 id `200001`，职业 `shuai`）：所在阵营存在携带主公技的英雄时，**该阵营护盾效果加倍**（`MasterShieldDouble = 2`）。
...
## 七、验证点
1. 3/5/7/9 同阵营英雄分别获得 18%/24%/30%/36% 护盾；王在场翻倍。
